...
$prefill_ride_id = isset($_GET['ride_id']) ? (int)$_GET['ride_id'] : 0;

if ($prefill_ride_id) {
    $q = "SELECT r.ride_id, r.fare, r.pickup_location, r.drop_location,
                 r.status, r.rider_id, r.driver_id
          FROM rides r WHERE r.ride_id = $prefill_ride_id";
    $res = $conn->query($q);
    if ($res && $res->num_rows > 0) {
        $ride_info = $res->fetch_assoc();
    }
}
...
$filter_rider = isset($_GET['rider_id']) ? (int)$_GET['rider_id'] : 0;

if ($filter_rider) {
    $hSql = "SELECT 
                p.payment_id, p.ride_id, p.amount, p.payment_method,
                p.payment_status, p.created_at AS payment_date,
                r.pickup_location, r.drop_location, r.status AS ride_status
             FROM payments p
             JOIN rides r ON p.ride_id = r.ride_id
             WHERE r.rider_id = $filter_rider
             ORDER BY p.created_at DESC";
    $hRes = $conn->query($hSql);
    while ($hRow = $hRes->fetch_assoc()) $history[] = $hRow;
}
...
<header class="header">
    ...
    <nav>
        <ul class="nav-links">
            ...
            <li><a href="make_payment.php" class="active">Payment</a></li>
            <li><a href="submit_rating.php">Rating</a></li>
        </ul>
    </nav>
</header>
        ...
        <div>
            <div class="panel">
                ...
                <div class="panel-body">
                    ...
                    <?php if ($success_data): ?>
                        <div class="alert success">✓ Payment processed successfully!</div>
                        <div class="receipt">
                            <div class="receipt-header">
                                <div class="tick">✅</div>
                                <h3>Payment Confirmed</h3>
                                <p>Transaction #<?= $success_data['payment_id'] ?></p>
                            </div>
                            <div class="receipt-body">
                                <div class="receipt-row highlight">
                                    <span class="rr-label">Amount Paid</span>
                                    <span class="rr-val">৳<?= number_format($success_data['amount'], 2) ?></span>
                                </div>
                                <div class="receipt-row">
                                    <span class="rr-label">Ride #</span>
                                    <span class="rr-val">#<?= $success_data['ride_id'] ?></span>
                                </div>
                                <div class="receipt-row">
                                    <span class="rr-label">From</span>
                                    <span class="rr-val"><?= htmlspecialchars($success_data['pickup']) ?></span>
                                </div>
                                <div class="receipt-row">
                                    <span class="rr-label">To</span>
                                    <span class="rr-val"><?= htmlspecialchars($success_data['drop']) ?></span>
                                </div>
                                <div class="receipt-row">
                                    <span class="rr-label">Method</span>
                                    <span class="rr-val">
                                        <?= methodIcon($success_data['payment_method']) ?>
                                        <?= ucfirst(htmlspecialchars($success_data['payment_method'])) ?>
                                    </span>
                                </div>
                                <div class="receipt-row">
                                    <span class="rr-label">Status</span>
                                    <span class="rr-val" style="color:var(--green);">✓ Paid</span>
                                </div>
                            </div>
                        </div>
                        <br>
                        <a href="submit_rating.php?ride_id=<?= $success_data['ride_id'] ?>&driver_id=<?= $success_data['driver_id'] ?>&rider_id=<?= $success_data['rider_id'] ?>"
                           style="display:block;text-align:center;color:var(--gold);text-decoration:none;font-size:0.9rem;font-weight:600;margin-top:0.5rem;">
                           ⭐ Rate your driver →
                        </a>
                    <?php else: ?>
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="form-group">
                                <label>Ride ID</label>
                                <input type="number" name="ride_id" placeholder="Enter Ride ID"
                                       value="<?= $prefill_ride_id ?: '' ?>" required>
                            </div>

                            <?php if ($ride_info): ?>
                                <div class="ride-preview">
                                    <div class="rp-label">Ride Preview</div>
                                    <div class="rp-route">
                                        <div class="rp-loc">
                ...
                <div class="panel-body">
                    <form method="GET" action="">
                        <div class="hist-filter">
                            <div style="flex:1; min-width:130px;">
                                <label class="hf-label">Rider ID</label>
                                <input type="number" name="rider_id" placeholder="e.g. 1"
                                       value="<?= htmlspecialchars($_GET['rider_id'] ?? '') ?>">
                            </div>
                            <?php if ($prefill_ride_id): ?>
                                <input type="hidden" name="ride_id" value="<?= $prefill_ride_id ?>">
                            <?php endif; ?>
                            <button type="submit" class="hist-btn">🔍 Load</button>
                        </div>
                    </form>

                    <?php if ($filter_rider && count($history) === 0): ?>
                        <div style="text-align:center;padding:2rem;color:var(--muted);">
                            <div style="font-size:2rem;margin-bottom:0.5rem;">💳</div>
                            <div style="font-weight:600;">No payments found</div>
                            <div style="font-size:0.82rem;margin-top:0.2rem;">Rider #<?= $filter_rider ?> has no payment records.</div>
                        </div>
                    <?php elseif (count($history) > 0): ?>
                        ...
                        <table class="hist-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td style="color:var(--accent);font-weight:700;"><?= $h['payment_id'] ?></td>
                                    <td style="font-family:'Syne',sans-serif;font-weight:700;color:var(--accent2);">#<?= $h['ride_id'] ?></td>
                                    <td style="font-size:0.82rem;">
                                        <span style="color:var(--green);">↑</span>
                                        <?= htmlspecialchars(mb_strimwidth($h['pickup_location'],0,18,'…')) ?><br>
                                        <span style="color:var(--red);">↓</span>
                                        <?= htmlspecialchars(mb_strimwidth($h['drop_location'],0,18,'…')) ?>
                                    </td>
                                    <td style="font-family:'Syne',sans-serif;font-weight:700;color:var(--green);">
                                        ৳<?= number_format($h['amount'], 2) ?>
                                    </td>
                                    <td>
                                        <span class="paid-badge">
                                            <?= methodIcon($h['payment_method']) ?>
                    ...
                    <?php else: ?>
                        <div style="text-align:center;padding:2.5rem;color:var(--muted);">
                            <div style="font-size:2rem;margin-bottom:0.5rem;">🔎</div>
                            <div style="font-size:0.88rem;">Enter a Rider ID to view their payment history.</div>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
